...
public class FormattedTextEventChatFinder {
    public static void find(String className, byte[] classFileBuffer) {
        ...
        if (match) {
            Debug.println("[FormattedTextEventChatFinder] Found: " + className);

            Fields.EVENT_BUS_FORMATTED_MESSAGE_EVENT_CLASSES_NAME = className;
        }
    }

    private static void addSet(Set<String> set) {
        set.add("getType");
        set.add("getFormattedMessage");
    }
}
